...
namespace Echo
{
	namespace PlaneVeg
	{
		...
		Vector3 sphPosFloat((float)mSphPos.x, (float)mSphPos.y, (float)mSphPos.z);
		return AxisAlignedBox(mBox.getMinimum() + sphPosFloat, mBox.getMaximum() + sphPosFloat); 
	};
		...
		void						setRenderData(const Vector3& sphPos, const DVector3& camPos) {
			mSphPos = DVector3(sphPos);  // Convert to DVector3 for precision
			mCamPos = camPos;
		...
		private:
			...
			DVector3		mSphPos = DVector3::ZERO;  // Use DVector3 for precision
