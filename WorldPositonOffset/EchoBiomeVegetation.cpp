...
namespace Echo
{
	namespace SphVeg
	{
		...
		void						setSphPos(const DVector3& pos) { setRenderData(pos, mCamPos); };
		void						setRenderData(const DVector3& sphPos, const DVector3& camPos)
		{
			// [CR FIX] Convert to camera-relative coordinates
			DVector3 worldOrigin = Root::instance()->getWorldOrigin();
			mSphPos = sphPos - worldOrigin;
			mCamPos = camPos;
		...
		DVector3		mSphPos = DVector3::ZERO;  // [CR] Store as relative coordinate
		...
		void						setDrawData(const DVector3& planetPos, const DVector3& camPos, bool preZ) {
			// [CR FIX] Convert to camera-relative coordinates
			DVector3 worldOrigin = Root::instance()->getWorldOrigin();
			mSphPos = planetPos - worldOrigin;
			mCamPos = camPos;
			mPreZ = preZ;
			...
			// [CR FIX] Explicit conversion from DVector3 to Vector4(float)
	...
	// [CR FIX] Calculate camera-space offset (both are relative coordinates now)
	...
	DVector3 mSphSpaceCamPos = DVector3::ZERO;  // [CR] Use DVector3 for precision storage
	Vector3 mSphSpaceCamPosFloat = Vector3::ZERO;  // [CR] Float version for consistent distance checks
	...
	// [CR FIX] Calculate sphere-space camera position using relative coordinates
	DVector3 worldOrigin = Root::instance()->getWorldOrigin();
	DVector3 sphPosRelative = _dp->mInfo.sphPos - worldOrigin;
	DVector3 camPosRelative = pCamera->getPosition();  // Already relative from Camera CR fix
	_dp->mSphSpaceCamPos = camPosRelative - sphPosRelative;
	
	// [CR FIX] Create float version for consistent distance/visibility checks
	// This is safe because mSphSpaceCamPos is already in sphere-space (small values)
	_dp->mSphSpaceCamPosFloat = Vector3(
		(float)_dp->mSphSpaceCamPos.x, 
		(float)_dp->mSphSpaceCamPos.y, 
		(float)_dp->mSphSpaceCamPos.z);
	
	const Plane* camPlane = pCamera->getFrustumPlanes();
	for (uint32 i = 0; i < 6; i++)
	{
		_dp->mSphSpaceCamPlanes[i] = camPlane[i];
		// [CR FIX] Explicit conversion from double to float
		float dotProduct = (float)(sphPosRelative.x * _dp->mSphSpaceCamPlanes[i].normal.x + 
		                            sphPosRelative.y * _dp->mSphSpaceCamPlanes[i].normal.y + 
		                            sphPosRelative.z * _dp->mSphSpaceCamPlanes[i].normal.z);
		_dp->mSphSpaceCamPlanes[i].d += dotProduct;
	}
...
{
	if (pos == (Vector3)_dp->mInfo.sphPos)
		return;

	_dp->mInfo.sphPos = DVector3(pos);
	...
	namespace SphVeg
	{
		...
		// [CR FIX] Use consistent float version for distance checks to avoid precision-induced jitter
		const float camToBoxNearSquareDis = nodeBox.getDisSqrt(_dp->mSphSpaceCamPosFloat, false);
		...
		// [CR FIX] Convert sphere position to camera-relative
		DVector3 worldOrigin = Root::instance()->getWorldOrigin();
		mSphPos = _dp->mInfo.sphPos - worldOrigin;
	...
	void _Render::setCustomParameters(const Pass* pPass)
	{
		...
		bool _AsyncTask::task_executionImpl()
		{
			...
			for (int aIndex = 0; aIndex < 3; aIndex++)
			{
				...
				for (const auto& [p, terrainIndex] : randomData)
				{
					...
					// [CR FIX] Use double precision for world position calculation
					DVector3 worldSpacePos = DVector3::ZERO;

					if (dyInfoPtr->sphPtr->isSphere())
					{
						const Vector3 cubeSpacePos = _FacePosToCube(id.face, dyInfoPtr->sphRadius, Vector2(p.x, p.y));
						const Vector3 pDir = cubeSpacePos.GetNormalized();
						worldSpacePos = dyInfoPtr->sphPos + DVector3(pDir) * (double)dyInfoPtr->sphRadius;
					}
					else
					{
						const Vector3 ringNorPos = _FacePosToRingNor(id.face, dyInfoPtr->sphPtr, Vector2(p.x, p.y), faceSize, 0.f);
						worldSpacePos = dyInfoPtr->sphPos + DVector3(ringNorPos);
					}

					Vector3 hitNormal = Vector3::UNIT_Y;
					const DVector3 hitPos = _DyGetSurfaceWs(dyInfoPtr->sphPtr, worldSpacePos, &hitNormal);

				{
					// Convert to Vector3 for road check (protocol expects Vector3)
					Vector3 hitPosFloat = Vector3((float)hitPos.x, (float)hitPos.y, (float)hitPos.z);
					roadPackage.elements.front() = { hitPosFloat, true };
					...
					{
						if (sRecordFlag)
							sRecordRoad.push_back(hitPosFloat);
						continue;
					}
				}

				// [CR FIX] Calculate sphere-space position with double precision, then convert to float
				const DVector3 sphSpaceHitPos = hitPos - dyInfoPtr->sphPos;

				_InsData insData = { .pos = Vector4((float)sphSpaceHitPos.x, (float)sphSpaceHitPos.y, (float)sphSpaceHitPos.z, scaleValue) };
		...
		void  _RenderCachePool::findVis(_NodeID id, const FloatRect& nodeRange, const Camera* pCamera, RenderQueue* pQueue, float fCurTime, _Private* _dp, const Matrix3& invBase)
		{
			...
			// [CR FIX] Use consistent float version for distance checks to avoid precision-induced jitter
			if (lodBox.getDisSqrt(_dp->mSphSpaceCamPosFloat, false) >= lodShowDis * lodShowDis)
				continue;
			if (false == lodBox.isVisible(_dp->mSphSpaceCamPlanes.data(), false))
				...
				// [CR FIX] Use consistent float version for distance checks to avoid precision-induced jitter
				if (renderBox.getDisSqrt(_dp->mSphSpaceCamPosFloat, false) >= renderShowDis * renderShowDis)
					continue;
				if (false == renderBox.isVisible(_dp->mSphSpaceCamPlanes.data(), false))
					continue;

				// [CR FIX] Use consistent float version for density calculation
				float squaDis = (float)(renderBox.toObSpace(_dp->mSphSpaceCamPosFloat) - (renderBox.min() + renderBox.max()) / 2.f).length();
