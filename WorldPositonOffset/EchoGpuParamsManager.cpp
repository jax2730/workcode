...
namespace Echo
{
	...
	void GpuParamsManager::commitUniform(uint32 uniformNameID, void* pBuffer, uint32 data_size)
	{
		...
		{
			// Build camera-relative world matrix and output as 3 float4 rows
			Matrix4 tempMat = *m_pWorldMatrix;
			Camera* pCam = m_sceneManager->getActiveCamera();
			const DVector3& cPos = pCam->getPosition();
			tempMat[0][3] -= static_cast<float>(cPos.x);
			tempMat[1][3] -= static_cast<float>(cPos.y);
			tempMat[2][3] -= static_cast<float>(cPos.z);
			for (size_t i = 0; i < 3; ++i)
			{
				const float* p = tempMat[i];
				pVector4->x = *p++;
	...
	/*	case U_VS_CameraPosition:
		...
		case U_VS_CameraPosition:
		{
			const Vector3& camWorld = m_sceneManager->getActiveCamera()->getPosition();
			pVector4->x = camWorld.x;
			pVector4->y = camWorld.y;
			pVector4->z = camWorld.z;
			pVector4->w = 1.0f;
		}
		break;
		...
		case U_LightWorldPosiArray:
		{
			...
			// Make light positions camera-relative to match PS using CR world positions
			const Vector3 camWorld = m_sceneManager->getActiveCamera()->getPosition();
			for (size_t i = 0; i < maxPointLightNum; i++)
			{
				...
				pVector4->x = lightPosi.x - camWorld.x;
				pVector4->y = lightPosi.y - camWorld.y;
				pVector4->z = lightPosi.z - camWorld.z;
