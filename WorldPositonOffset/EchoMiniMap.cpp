...
//#endif
	...
	void MiniMapManager::SetShowFullMap(bool InVal)
	{
		...
		if (mbShowFullMap)
		{
			...
			{
				mCurCityUI.mObject->as<echofgui::GComponent>()->addChild(mCloseFullMapUI.mObject);

				float buttonWidth = mCloseFullMapUI.mObject->getWidth() * ADAPTATION_SCALE_X;
				float margin = 8.0f;
				float mapWidth = mMapRangeUI.mObject->getWidth();
				mCloseFullMapUI.mObject->setPosition(RealStartPosX + mapWidth - buttonWidth - margin, RealStartPosY + margin);
				mCloseFullMapUI.mObject->setScale(ADAPTATION_SCALE_X, ADAPTATION_SCALE_Y);
				mCloseFullMapUI.mObject->setTouchable(true);
		...
		else
		{
			...
			{
				mChangeFullMapUI.mObject->setVisible(true);
				mChangeFullMapUI.mObject->setTouchable(true);
			}
			if (mCloseFullMapUI.mObject)
			{
				mCloseFullMapUI.mObject->setTouchable(false);
				mCloseFullMapUI.mObject->setVisible(false);
				mCloseFullMapUI.mObject->removeFromParent();
			}
		}
