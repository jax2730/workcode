...
#include <Common_Function.inl>
	...
	float4  U_WorldMatrixBaseView[3];
#endif
...
VS_OUTPUT main(VS_INPUT IN, uint i_InstanceID : SV_InstanceID)
{
	...
	// matInvTransposeWorld needs world-space position for correct calculation
	float3 vWorldPos = vPosition.xyz;
	vWorldPos.x += U_VS_CameraPosition.x;
	vWorldPos.y += U_VS_CameraPosition.y;
	vWorldPos.z += U_VS_CameraPosition.z;
	...
	matWorld[0] = U_WorldMatrixBaseView[0];
	matWorld[1] = U_WorldMatrixBaseView[1];
	matWorld[2] = U_WorldMatrixBaseView[2];

	float4x4 world4x4;
world4x4[0] = float4(matWorld[0]);          
world4x4[1] = float4(matWorld[1]);
world4x4[2] = float4(matWorld[2]);
world4x4[3] = float4(0.0, 0.0, 0.0, 1.0);
	...
	matWVP = mul((float4x4)U_ZeroViewProjectMatrix, world4x4);
	...
	// Build a rotation-only matrix for normal/tangent transform to avoid CR translation noise
	float4x4 worldRot;
	#ifdef U_WorldRotationMatrix
		worldRot[0] = U_WorldRotationMatrix[0];
		worldRot[1] = U_WorldRotationMatrix[1];
		worldRot[2] = U_WorldRotationMatrix[2];
		worldRot[3] = float4(0.0,0.0,0.0,1.0);
	#else
		worldRot[0] = float4(matWorld[0].xyz, 0.0);
		worldRot[1] = float4(matWorld[1].xyz, 0.0);
		worldRot[2] = float4(matWorld[2].xyz, 0.0);
		worldRot[3] = float4(0.0,0.0,0.0,1.0);
	#endif
	...
	// Use rotation-only matrix to derive stable world-space basis in CR space
	vsOut.o_Normal.xyz = mul(worldRot, normal);
	vsOut.o_Tangent.xyz = mul(worldRot, tangent);
	...
	float3 camToWorld = (vsOut.o_WorldPos.xyz + U_VS_CameraPosition.xyz) - U_VS_CameraPosition.xyz; // stable eye vector in world space equals -o_WorldPos
		...
		vsOut.o_PosInClip.w = dot(U_WorldViewProjectMatrix[3], tempLocalPosi);U_ZeroViewProjectMatrix[4]
	#endif
	...
	#ifdef USESHADOWMAP
		// Convert CR world position back to world-space for shadow computations
		float4 worldPosWS = float4(vsOut.o_WorldPos.xyz + U_VS_CameraPosition.xyz, 1.0f);
		float viewZ = -dot(U_ViewMatrix[2], worldPosWS);
	
	    float4 wpos = worldPosWS;
