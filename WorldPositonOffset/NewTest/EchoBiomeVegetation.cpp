...
namespace Echo
{
	namespace SphVeg
	{
		...
		public:
			...
			void					setSphPos(const DVector3& pos) { setRenderData(pos, mCamPosWorld); };
		...
		std::array<Plane, 6> mSphSpaceCamPlanes;			bool mUsePointLight = true;	//是否使用点光源
	...
	DVector3 camPosRelative = pCamera->getPosition();
	_dp->mSphSpaceCamPos = camPosRelative - _dp->mInfo.sphPos;

	// Create float version for consistent distance/visibility checks
	...
	for (uint32 i = 0; i < 6; i++)
	{
		...
		float dotProduct = (float)(_dp->mInfo.sphPos.x * _dp->mSphSpaceCamPlanes[i].normal.x +
			_dp->mInfo.sphPos.y * _dp->mSphSpaceCamPlanes[i].normal.y +
			_dp->mInfo.sphPos.z * _dp->mSphSpaceCamPlanes[i].normal.z);
