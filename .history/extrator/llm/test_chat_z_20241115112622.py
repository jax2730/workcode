...
def get_state(state):
    ...
    if isinstance(messages[-1], AIMessage) and messages[-1].tool_calls and "确认" == messages[-1].content:
        print("-------------------确认---------------")
...
def get_biome_docs():
    biome_data_docs = []
    for record in preload.biomeData.get_all_records():  # 假设 get_all_records 返回温湿度范围和索引
        ...
        biome_name = [
            name for name, index in preload.originBiomeMap.items() 
    ...
    print("------------all_docs-------------", biome_data_docs)
    return biome_data_docs

    # origin_biome_docs = [
    #     Document(
    #         page_content=f"Biome name: {name}, Index: {index}",
    #         metadata={"name": name, "index": index}
    #     ) for name, index in preload.originBiomeMap.items()
    #     ]
    # all_docs = biome_data_docs + origin_biome_docs
...
def chat(humanMsg:str, session_id:str):
    ...
    if name and args:
        ...
        # 查询 FAISS 索引
        query = humanMsg  # 或者根据 args 构造一个更具体的查询
