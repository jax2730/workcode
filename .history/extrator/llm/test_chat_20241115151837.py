...
# 获取之前的会话历史
    ...
    # 提取当前状态中的参数
    temperature_range = state.get("temperature_range")
    humidity_range = state.get("humidity_range")
    color = state.get("color")
    
    # 检查是否所有必要的参数都已提取
    if temperature_range is None:
        return {"messages": ["请提供温度范围"]}  # 返回一个提示让用户补充温度范围
    elif humidity_range is None:
        return {"messages": ["请提供湿度范围"]}  # 返回一个提示让用户补充湿度范围
    elif color is None:
        return {"messages": ["请提供颜色信息"]}  # 返回一个提示让用户补充颜色信息
    
    # 如果所有必要的参数都已经提供，则继续调用工具
    parse_fresult = argumentParser.get_format_instructions()
    response = extractor.invoke(
        {
            "format_instraction": parse_fresult,
            "new_history": state["messages"],
            "history": session_history.messages  # 保持会话历史
        },
        config=config
    )
    ...
    # 进行tool calls
    tool_response = tool_calls(state)
