...
def get_state(state):
    ...
    for message in messages:
        if isinstance(messages[-1], ToolMessage) and isinstance(message, ToolMessage):
            if message.need_modify == True:
                return "recheck"
...
def add_tool_message(state: State):
    ...
    need_modify,subRange, content = set_env(tool_args)
...
def recheck(state: State):
    ...
    print("当前节点-----------recheck----------------") 
    extracted_colors = [
        tool_call['function']['arguments']['colors']
        for message in  state['messages']
        if isinstance(message, AIMessage)
        for tool_call in message.response_metadata['message']['tool_calls']
    ]
    for message in  state['messages']:
        if isinstance(message, ToolMessage):
            if hasattr(message, 'need_modify'):
                subRange = mod_subMap()
                # print("submap_info:", message.submap_info)
               
                message.need_modify = False
    content = "修改部分内容"
    return {
        "messages": [
            AIMessage(
...
# workflow.add_conditional_edges("recheck", get_state,["info", "recheck", END])
workflow.add_conditional_edges("add_tool_message", get_state,["recheck", "info", END])
workflow.add_conditional_edges("recheck", get_state,["recheck", "add_tool_message", END])
...
def set_env(args):
    ...
    if have_idx0:
        # global vectorstore
        # # 查询 FAISS 索引
        # docs = vectorstore.similarity_search(args, k=10)  # 查询最相关的 5 条记录
        ...
        return have_idx0, subRange, user_message
    return have_idx0, subRange, "环境设置完成"

def mod_subMap(args, subRange):
    colorAssets = preload_1.filterByColor(args["colors"])
