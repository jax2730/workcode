...
def get_state(state):
    ...
    #     if isinstance(messages[-2], ToolMessage) and isinstance(message, ToolMessage):
    #         if message.need_modify == True:
    #             return "recheck"
    # 检查最后一条消息是否为 `ToolMessage` 且需要修改
    if isinstance(messages[-1], ToolMessage) and messages[-1].need_modify:
        return "recheck"
...
workflow.add_conditional_edges("info", get_state,["add_tool_message", "recheck", "info", END])
# workflow.add_conditional_edges("recheck", get_state,["info", "recheck", END])
workflow.add_edge("add_tool_message", "info")
workflow.add_edge("recheck", "info")
...
def chat(humanMsg:str, session_id:str):
    ...
    #         if isinstance(message, ToolMessage):
    #             if hasattr(message, 'submap_info'):
    #                 mod_subMap(args, message.submap_info)
