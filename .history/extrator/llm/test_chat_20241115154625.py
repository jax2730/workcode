...
def set_env(state: State):
    ...
    if args:
        # 检查每个字段是否完整
        if "temperature" in args and "humidity" in args and "colors" in args:
            ...
            if (
                ...
                and "min" in temperature and "max" in temperature
                and isinstance(humidity, dict)
                and "min" in humidity and "max" in humidity
                and isinstance(colors, list)
                and len(colors) > 0
            ):
                # 所有字段都齐全且有效
                content = "环境设置完成，已成功配置参数。"
                return {
                    "messages": [
                        ToolMessage(
                            content=content,
                ...
                # 字段存在但不完整或无效
                content = "环境参数不完整或无效，请补充完整后重试。"
        else:
            # 某些字段缺失
            content = "缺少必要的参数（温度、湿度或颜色），请补充后重试。"
    else:
        # 如果没有工具调用参数
        content = "没有工具调用参数，请提供环境设置参数后重试。"
